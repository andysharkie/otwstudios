// lib/gmail.js â Gmail API helpers (OAuth token management + email fetch + send + labels)
 ...
 */
export async function listGmailLabels(clientId) {
  const auth  = await getAuthedClient(clientId);
  const gmail = google.gmail({ version: 'v1', auth });
...
/**
 * Create a new Gmail label and return the created label object
 */
export async function createGmailLabel(clientId, name) {
  const auth  = await getAuthedClient(clientId);
  const gmail = google.gmail({ version: 'v1', auth });
 ...
 */
export async function applyLabelToMessage(clientId, messageId, gmailLabelId) {
  const auth  = await getAuthedClient(clientId);
  const gmail = google.gmail({ version: 'v1', auth });
 ...
 */
export async function removeLabelFromMessage(clientId, messageId, gmailLabelId) {
  const auth  = await getAuthedClient(clientId);
  const gmail = google.gmail({ version: 'v1', auth });
  await gmail.users.messages.modify({
    userId: 'me',
